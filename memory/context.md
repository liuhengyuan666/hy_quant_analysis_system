...
- [InProgress] [TASK-100] Regime Attribution Study (Failure Classification): Build a failure taxonomy for Signal/State divergences (e.g., Liquidity Trap, Crowding, Theme Rotation, Macro Shock, Breadth Collapse, Momentum Exhaustion). Track each Shadow Production divergence case and classify it. Goal is to accumulate a Failure Knowledge Base, not to improve Signal Accuracy.
...
- [Todo] [TASK-222] RV1-gated residual: persist deterministic ExecutionEvent/ExecutionResearchRecord as a durable Research Asset through the existing WorkspaceManager lifecycle. This is not a new execution architecture or decision path. Blocked from implementation until TASK-111 gates are met: 1000+ Research Assets, 30-day replay stability, and 2-cycle calibration stability; must preserve ADR-104 capability consolidation, ADR-105 domain freeze, and ADR-106 LLM explanation-only boundary.
- [Todo] [TASK-224] P0/P1 Research Asset Provenance: extend existing WorkspaceManager AssetMetadata with backward-compatible factual provenance and truthful created_at/generated_at/producer semantics; populate bounded replay provenance without creating a parallel asset system, rewriting legacy assets, adding scores/weights, or wiring TASK-222.
- [Todo] [TASK-225] P1 Path-Based Evaluation Foundation: add pure core-domain path calculations for T+5/T+20/T+60/T+120, MFE, MAE, MaxDD, time-to-MFE, giveback, and benchmark excess; make outcome resolution honor explicit as_of. Measurements only—no thresholds, classifications, portfolio decisions, or ledger schema expansion initially.
- [Todo] [TASK-226] P1 Episode-Based Evaluation Read Model: derive scope-date episodes and conservative-state runs from existing divergence ledger CaseKey records and state history; preserve Prospective/Reconstructed separation and nested symbol membership. Do not persist synthetic episode IDs, create new Research Asset kinds, or auto-classify TASK-100 records.
- [Todo] [TASK-227] P1 Evidence Semantic Integrity: represent unavailable leverage as Unknown/Unavailable instead of Normal and prevent missing data from contributing zero-risk semantics; preserve numeric breadth facts while qualifying consumer-specific labels by producer/schema. Requires a Proposed ADR before changing persisted or shared enums; no threshold changes.
