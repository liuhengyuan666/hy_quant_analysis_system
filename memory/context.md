...
- [InProgress] [TASK-096] Add --date support to `research srd` and `research stretch` commands. Use unified ResearchDateOptions { scope, date } parsing. Default to latest date when --date is omitted. This enables historical date research output for validation and Quarterly Review generation without adding new CLI tools.
- [Todo] [TASK-097] Abstract an internal `ResearchSnapshot` model that holds SRD, Stretch, Rotation, Breadth, and Analytics results for a single (date, scope). Refactor existing research commands to build ResearchSnapshot first, then render. This unifies query logic and enables Quarterly Review aggregation. No new CLI.
- [Todo] [TASK-098] Design and implement Research Quarterly Review: automatically run SRD/Stretch/Analytics over a 90-day window and generate a Markdown report (Observation Window, SRD summary, Stretch distribution, top findings, potential ADR candidates). Output to reports/research-quarterly-{scope}-{date}.md. This is the synthesis layer that turns Observation + Analytics into accumulated research assets.
