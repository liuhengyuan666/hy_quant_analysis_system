...
- [Todo] [TASK-212] ADR-113 #4: adversarial_diag 诊断元数据丰富化。LlmAnalysisRecord 加 #[serde(default)] source 字段（market-refresh/on-demand，老记录兼容）；响应 JSON adversarial_diag 扩充为 enabled/injected/level/fresh/generated_at/source；LlmAnalysisPanel 渲染诊断（含未注入原因提示，解决用户不知道博弈层是否工作的问题）。
- [Todo] [TASK-213] [P4] ADR-113 #5: 前置/主 LLM 调用复用 HTTP client（async_openai::Client 或 reqwest::Client 共享，消除冷路径第二次 TLS 握手，收益 ~50-200ms）。优先级最低，等 TASK-210/211/212 落地后顺手做。
- [Todo] [TASK-214] ADR-113/114 #1 REVISED: market-refresh 预生成 adversarial 为异步 warm cache。snapshot ready 后 spawn 后台 preparation 任务写 llm-history（source=market-refresh），失败静默，绝不阻塞或失败 refresh 主流程。auto_inject=true 时启用，false 总开关同时关闭注入与预生成。验证：刷新完成后首次 llm-analyze 走热路径 fresh=true。
- [Todo] [TASK-215] ADR-114: InjectionLevel 与 ContentPolicy 解耦实现。Level 语义：compact=record.summary 结构化摘要 / standard=analysis_text 默认正文（policy 保护 max_chars=4000）/ full=analysis_text 完整（更高硬顶保护）；ContentPolicy 独立配置 [llm.adversarial] max_chars + truncate_strategy=paragraph_boundary，截断头部标注 + provenance 字段 original_chars/final_chars/truncated。Wave 0 先做词表（summary→compact + 新增 standard），机制本任务落地。
