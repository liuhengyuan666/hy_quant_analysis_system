...
| Native command surface | `src-tauri/src/lib.rs` | `app_status`, `dashboard_snapshot`, `data_health_summary`, `recent_reports`, `usage_guides`, async refresh commands |
...
- Health-check summary and health-report export are now exposed in desktop UI.
...
- Help / Usage docs are now in-app, loaded from project markdown files through Tauri.
