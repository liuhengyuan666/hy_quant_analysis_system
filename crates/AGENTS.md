...
├── app-service/        # orchestration facade, trust assembly, refresh guards, V6 research commands
...
| Change trust / dashboard bootstrap / recent reports | `app-service/` + `report-engine/` | `app-service` assembles (orchestration monolith with helper modules: `core`, `trust`, `breadth`, `dashboard`, `llm`, `sync`, `config_loader`); `report-engine` shapes |
...
- `app-service/src/lib.rs` is still the orchestration monolith (~4,890 lines), but helper logic has been extracted into `core.rs`, `trust.rs`, `breadth.rs`, `dashboard.rs`, `llm.rs`, `sync.rs`, `config_loader.rs`. Review nearby helper modules before adding more flow logic.
...
- `regime-audit/src/lib.rs` is the largest single-file analysis crate (~3,057 lines); consider decomposition if adding more audit modes.
